...
#: The APP root — ``app/``, the folder `resources/app.yml` hands to Databricks
#: Apps as its `source_code_path`. `server/` sits one level below it, next to
#: `client/`, `dist/` and `shared/`. A relative `DBX_FRONTEND_DIST` is
#: resolved against this, not against the process's cwd, which on Databricks
#: Apps is not something this code chose.
