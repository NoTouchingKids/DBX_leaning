...
- **It is a client-rendered SPA, and cannot be anything else.** Databricks
  ...
  build this. `app/server/spa.py` serves the built bundle: it mounts `dist/assets`
...
serves the built bundle itself, the way `app/server/spa.py` does in a deploy, so
...
- `app/client/src/transport/hub.ts` — the header, then the tests
- `app/client/src/components/models/contract.ts` — before writing any model view
- `docs/message-envelope-spec.md` — the four message types, in full
- `app/server/routes/stream.py` — the SSE contract: `id:` is the message's `seq`, so
...
- `app/server/routes/runs.py` — trigger, backfill (`GET /api/runs/{id}/messages`),
  results, and cancel
