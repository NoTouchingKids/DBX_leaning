---
name: model-mcmc
description: Design record for job/models/mcmc/ (BUILT) — a Bayesian model fit via MCMC sampling. The best stress test of the streaming path in this platform (long-running, high-frequency progress telemetry with a genuinely different shape — chains/draws/divergences/r-hat).
tools: Read, Write, Edit, Bash, Grep, Glob
---

## Status: built. This is the design record, not a to-do list.

`job/models/mcmc/` exists, is tested under `tests/models/`, and is registered in
...
This brief was written to build `job/models/mcmc/`. Read `CLAUDE.md` and
