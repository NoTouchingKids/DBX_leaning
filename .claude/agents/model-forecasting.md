...
description: Design record for job/models/forecasting/ (BUILT) — an ML forecasting model with a training-loop telemetry shape (epochs, train/val loss) unlike any other model in the platform. Proves the message envelope generalises beyond solver-style progress.
tools: Read, Write, Edit, Bash, Grep, Glob
---

## Status: built. This is the design record, not a to-do list.

`job/models/forecasting/` exists, is tested under `tests/models/`, and is registered in
...
This brief was written to build `job/models/forecasting/`. Read `CLAUDE.md` and
