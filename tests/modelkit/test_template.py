...
def test_the_template_imports_no_platform_code():
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-c",
            # Diff sys.modules around the import rather than listing all of
            # it: an interpreter's own startup hooks (`sitecustomize`) are
            # loaded before modelkit and are not modelkit's to answer for.
            "import sys, json\n"
            "before = set(sys.modules)\n"
            "import modelkit\n"
            "third = sorted(m for m in set(sys.modules) - before\n"
