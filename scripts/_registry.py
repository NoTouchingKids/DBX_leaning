...
`[tool.dbx-leaning.models]` maps each package under `models/` to the
...
class UnregisteredModel(KeyError):
    """A models/ package with no entry in the registry."""
...
def model_extras() -> dict[str, str]:
    ...
    if not registry:
        raise UnregisteredModel(
            ...
            "every model needs an entry mapping its models/ directory to its extra"
        )
...
def extra_for(model: str) -> str:
    try:
        ...
        raise UnregisteredModel(
            ...
            f"add one (models/{model} -> its optional-dependencies extra) before it "
            f"can be packaged or deployed. Registered: {', '.join(model_names())}"
        ) from None


def discovered_packages() -> list[str]:
    """What is actually on disk under models/, registry or not."""
