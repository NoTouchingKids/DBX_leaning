...
class McmcModel:
    ...
    def build(self) -> None:
        ...
        self._start = start
        self._sampler = emcee.EnsembleSampler(self.n_chains, len(self.param_names), self.log_prob)
        self._log(
    ...
    def _chain_positions(self, chain) -> tuple[list[list[float]], bool]:
        ...
        return (
            [[round(float(v), 6) for v in walker] for walker in latest[:MAX_TRACE_CHAINS]],
            truncated,
        )
            ...
            rows.append(
                {
                    ...
                    "rhat": None
                    if not np.isfinite(rhat_by_param[i])
                    else round(float(rhat_by_param[i]), 6),
