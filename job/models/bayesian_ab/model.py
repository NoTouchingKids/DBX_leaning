...
``job/models/mcmc/`` is a sampler stress test: many chains, high-frequency draws,
...
Two comparisons, both derived from real observations in ``job/models/_data``
...
from .._data import Dataset, epoch_ms, nyc_taxi_hourly, nyc_taxi_trips
...
class BayesianAbModel:
    ...
    def _stage_lift_interval(self) -> None:
        a, b = self._posteriors()
        self.lift = difference_summary(a.alpha, a.beta, b.alpha, b.beta, mass=self.credible_mass)
