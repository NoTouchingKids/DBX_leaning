...
free ``samples`` catalog, via the shared loader in ``job/models/_data``. Off a
...
from .._data import Dataset, nyc_taxi_hourly
...
class StreamingResultsModel:
    ...
    def _ensure_data(self) -> None:
        ...
        self._log(
            f"backtest input: {data.provenance}; using {len(series)} points of '{self.column}'",
            phase="input",
        )
    ...
    def run(self) -> None:
        ...
        self._log(
            f"rolling-origin backtest: {total} windows, horizon {self.horizon}", phase="input"
        )
