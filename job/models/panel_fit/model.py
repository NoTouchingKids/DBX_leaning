...
class PanelFitModel:
    ...
    def __init__(self, config: dict[str, Any] | None = None) -> None:
        ...
        #: that has already streamed its rows (`job/models/README.md`).
    ...
    def _ensure_groups(self) -> list[_Group]:
        ...
        for row in data.rows:
            ...
            predictor = (
                period if cols.predictor == cols.period else _finite_float(row.get(cols.predictor))
            )
    ...
    def _terminal_status(self, total: int, r_squared_seen: list[float]) -> str | None:
        ...
        if self.groups_fitted == 0:
            ...
            self._log(
                ...
                "with nothing in it"
                if total
                else "the panel had no groups to fit",
                level="ERROR",
                phase="results",
            )
            return "INFEASIBLE"
