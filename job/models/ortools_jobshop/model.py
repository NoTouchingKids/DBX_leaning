...
**Why it is in the lineup**, next to `job/models/gurobi_scheduling` and
`job/models/gurobi_routing`:
...
#: (job/models/README.md, job/drivers/self_driving.py).
STATUS_RETURNS = ("INFEASIBLE", "FAILED")


class JobShopModel:
    #: Discovered by the harness (see job/models/README.md). The DDL for it is in
    ...
    def _status_return(self, status: Any, cp_model: Any) -> str | None:
        ...
        if status == cp_model.FEASIBLE:
            ...
            return f"feasible: makespan {self.makespan} min" + (
