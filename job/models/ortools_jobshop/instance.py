...
def bakery_batches(*, limit: int = 400, seed: int = 20260824) -> Dataset:
    ...
    Not in ``job/models/_data/datasets.py`` because that module is one function per
...
def _synthetic_batches(n: int, *, seed: int) -> list[dict[str, Any]]:
    ...
    rows.sort(key=lambda r: (-int(r["units"]), str(r["production_day"]), int(r["franchise_id"])))
    return rows
