...
``models/gurobi_scheduling/LICENCE_EXPIRY.md`` for the pin and its expiry.
...
class RoutingModel:
    #: Discovered by the harness (see models/README.md).
    ...
    def build(self) -> None:
        ...
        model.addConstr(
            gp.quicksum(depot_edge.values()) == 2 * inst.vehicles, name="depot_degree"
        )
    ...
    def routes(self) -> list[list[int]]:
        ...
        while ends:
            ...
            while True:
                step = next(
                    (w for w in sorted(adjacency[node]) if _key(node, w) not in used), None
                )
                if step is None:
                    break
