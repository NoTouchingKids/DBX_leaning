...
2. **A third shape of training telemetry.** ``models/forecasting`` already
...
``fare_amount``, ``duration_min`` (``models/_data``, ``nyc_taxi_trips``). Any
...
class NeuralNetModel:
    ...
    def _load(self) -> tuple[list[float], list[float]]:
        ...
        self._log(
            ...
            "excluded "
            + ", ".join(f"{col} ({why})" for col, why in EXCLUDED_COLUMNS.items()),
            phase="input",
        )
        ...
        usable = [
            (d, t) for d, t in zip(distance, duration, strict=True) if d > 0 and t > 0
        ]
    ...
    def build(self) -> None:
        ...
        features = np.stack(
            [dist, np.log1p(dist), np.sqrt(dist), 1.0 / dist], axis=1
        )
